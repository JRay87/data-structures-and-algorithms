...
const addExclamation = (arr) => {
  let newStringArr = [];
  arr.forEach((string) => newStringArr.push (string + '!'));
  return newStringArr;
};
...
const allUpperCase = (arr) => {
  let newCapsArray = [];
  arr.forEach((text) => newCapsArray.push(text.toUpperCase()));
  return newCapsArray;
};
...
const speaker = (words, greeting) => {
  let newCallBack = [];
  words.forEach((word) => newCallBack.push(greeting(word)));
  return newCallBack;
};
